...
static int parseArgs(int argc, char **argv, bv_args_t *args) {
    ...
    args->tChrom = argv[4];
    if (sscanf(argv[5], "%d", &args->tStart) != 1 || sscanf(argv[6], "%d", &args->tEnd) != 1) {
        return -1;
    }
    args->doSeq = 0;
    if (argc >= 8) {
...
static void printBlock(FILE *tFile, FILE *qFile, struct hal_block_t *b, const char *tChrom, size_t idx) {
    fprintf(tFile, "%s\t%ld\t%ld\t%ld\t0\t+\n", tChrom, b->tStart, b->tStart + b->size, idx);
    fprintf(qFile, "%s\t%ld\t%ld\t%ld\t0\t%c\n", b->qChrom, b->qStart, b->qStart + b->size, idx, b->strand);
}
...
int main(int argc, char **argv) {
    ...
    if (args.udcPath != NULL) {
        halGbUdcSetDefaultDir(args.udcPath);
    }
#endif
    ...
    if (handle >= 0) {
        ...
        struct hal_block_results_t *results =
            halGetBlocksInTargetRange(handle, args.qSpecies, args.tSpecies, args.tChrom, args.tStart, args.tEnd, 0, sm,
                                      HAL_QUERY_AND_TARGET_DUPS, 1, NULL, NULL);
